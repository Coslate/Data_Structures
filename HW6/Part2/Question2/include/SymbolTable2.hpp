...
int SymbolTable2<K>::hash_func(const K* in_key, const int key_size) const {
    if(key_size <= 0){
...
template <class K> //Only key, omitted element in this implementation
void SymbolTable2<K>::Insert(const pair<K> &in_pair){
    if(in_pair.size <= 0){
        throw std::runtime_error(std::string("Error: The key size cannot be smaller than or equal to 0. Insert() terminated."));
    }

    int nxt_bucket = 0;//will be the home bucket whether or not the item is found
    pair<K> *search_result = Search(in_pair.arr, in_pair.size, nxt_bucket);

    if(search_result == NULL){//not existed
        ht[nxt_bucket].InsertBack(in_pair);
...
template <class K> //Only key, omitted element in this implementation
bool SymbolTable2<K>::Compare(const K *arr1, const K *arr2, const int key_size1, const int key_size2) const {
...
template <class K> //Only key, omitted element in this implementation
pair<K>* SymbolTable2<K>::Search(const K *key, const int key_size, int &nxt_bucket) const {
    if(key_size <= 0){
    ...
    if(!ht[hashed_value].IsEmpty()){//not empty
        for(typename Chain<pair<K>>::Iterator j = ht[hashed_value].Begin(); j!=ht[hashed_value].End(); j++){
            if(Compare((*j).arr, key, (*j).size, key_size)){
                search_result = (j.operator->());
                break;
            }
        }
    }
...
template <class K> //Only key, omitted element in this implementation
void SymbolTable2<K>::Display() const {
    for(int i=0; i<ht_capacity; i++){
        ...
        if(ht[i].IsEmpty()){
            ...
            std::cout<<"\t";
            for(typename Chain<pair<K>>::Iterator j = ht[i].Begin(); j!=ht[i].End(); j++){
                std::cout<<*j<<" ";
            }
            std::cout<<std::endl;
        }
